...
void UniformBuffer::CreateDescriptorPool()
{
	...
	descriptorPoolCreateInfo.maxSets       = static_cast<uint32_t>(m_MaxFramesInFlight); // max descriptor sets that can be allocated
...
{
	std::vector<VkDescriptorSetLayout> descriptorSetLayouts(m_MaxFramesInFlight, m_GraphicsPipeline->GetDescriptorSetLayout());
